...
const useGetWithdraws = (userId: string, page: number) => {
	...
	return useQuery({
		queryKey: ["fetch-withdraws"],
		queryFn: () => getWithdraws(userId, page, jwtToken), // Pass jwtToken to getWithdraws
		// staleTime: Infinity,
...
const getWithdraws = async (userId: string, page: number, jwtToken: string) => {
	try {
		const response = await fetchWithAuth(
			`${
				import.meta.env.VITE_BACKEND_URL
			}/user/${userId}/get-withdraws?page=${page}&page_size=10`,
			{
				method: "GET",
				headers: {
...
export default useGetWithdraws;
