...
const useGetPnl = (userId: string) => {
	...
	return useQuery({
		queryKey: ["fetch-realized-pnl"],
		queryFn: () => getRealizedPnl(userId, jwtToken), // Pass jwtToken to getRealizedPnl
		// staleTime: Infinity,
...
const getRealizedPnl = async (userId: string, jwtToken: string) => {
	try {
		const response = await fetchWithAuth(
			`${import.meta.env.VITE_BACKEND_URL}/user/${userId}/get-realized-pnl`,
			{
				method: "GET",
				headers: {
