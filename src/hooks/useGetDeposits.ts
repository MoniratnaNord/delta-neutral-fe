...
const useGetDeposits = (userId: string, page: number) => {
	...
	return useQuery({
		queryKey: ["fetch-deposit-txns"],
		queryFn: () => getDepositTxns(userId, page, jwtToken), // Pass jwtToken to getDepositTxns
		// staleTime: Infinity,
...
const getDepositTxns = async (
	...
	try {
		const response = await fetchWithAuth(
			...
			}/user/${userId}/get-deposits?page=${page}&page_size=10`,
			{
				method: "GET",
				headers: {
